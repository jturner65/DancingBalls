...
public class myClrRamp {
	...
	final  int RGBIdx = 0, XYZIdx = 1, LABIdx = 2, LCHIdx = 3, USR1 = 4, USR2 = 5, USR3 = 6;
	...
	public myClrRamp(DancingBalls _p){
		...
		grads = new myClrGrad[numGrads];

		float y = pa.height *.15f;
		for(int i  =0; i<numGrads;++i){
			grads[i] = new myClrGrad(pa, this, pa.width *.85f, y, gradNames[i]);//(CAProject3 _pa, float _x, float _y){			
			y += 1.8f * pa.height * .07f;;
		}
	...
	public void drawRamp() {
		pa.pushMatrix(); pa.pushStyle();  // to ensure that we can restore the standard view before writing on the canvas
		// draw corners - need to scale based on size of box in 3D view
		pa.scale(scaleVals[0],scaleVals[1],scaleVals[2]);
		showColor(pa.black, 10);
		showColor(pa.red, 10);
		showColor(pa.blue, 10);
		showColor(pa.green, 10);
		showColor(pa.cyan, 10);
		showColor(pa.magenta, 10);
		showColor(pa.yellow, 10);
		showColor(pa.white, 10);		
		// draw edges of color cube
		cylinder(pa.black, pa.white);
		cylinder(pa.black, pa.red);
		cylinder(pa.black, pa.blue);
		cylinder(pa.black, pa.green);
		cylinder(pa.green, pa.yellow);
		cylinder(pa.green, pa.cyan);
		cylinder(pa.red, pa.yellow);
		cylinder(pa.red, pa.magenta);
		cylinder(pa.blue, pa.cyan);
		cylinder(pa.blue, pa.magenta);
		cylinder(pa.white, pa.magenta);
		cylinder(pa.white, pa.yellow);
		cylinder(pa.white, pa.cyan);	
		
		// draw the 2 user-controlled colors
		//int c0 = pa.color(R0,G0,B0), c1 = pa.color(R1,G1,B1);
		for(int i =0; i<usrPts.size();++i){
			usrPts.get(i).showColor();
		}	
	
//		//eventually only draw spline for chosen gradient
//		if(pa.flags[pa.clrAll]){
//			for(int i =0; i<this.grads.length; ++i){		grads[i].drawSpline();	}
//		}
//		else{
//			for(int i =0; i<this.grads.length; ++i){if(pa.flags[pa.clrRGB+i]){grads[i].drawSpline();}	}
//		}
...
//		drawLabRamp(c0,c1); // straight line ramp in Lab shown as curved in RGB
		...
		pa.popStyle();pa.popMatrix(); // done with 3D drawing. Restore front view for writing text on canvas
	  }//drawRamp
...
//		pa.msClkStr += "Click vals : mseX = " + String.format("%.5f", mseX) + " mseY = "+ String.format("%.5f", mseY);
	...
	public void updateGrads(){
		...
		clrPtsArrays = new ArrayList[numGrads];
		for(int i =0; i<numGrads; ++i){
			clrPtsArrays[i] = new ArrayList<myClrPoint>();
		}	
		
		myVector test1 = pa.U(p1, p2), test21, test22, test23, test31, test32, test33;
		myClrPoint pt01, pt02, pt03,
		pt41, pt42,  pt43 ;
		
		for(float s = 0; s <= 1.0f; s+=sIncr){		
			clrPtsArrays[RGBIdx].add(RGBrampPt(p1, p2, s));
			clrPtsArrays[XYZIdx].add(XYZrampPt(p1, p2, s));
			clrPtsArrays[LABIdx].add(LABrampPt(p1, p2, s));
			clrPtsArrays[LCHIdx].add(LCHrampPt(p1, p2, s));	
		}	
		...
		pt01 = new myClrPoint(pa, "kn1",XYZrampPt(p1, p2, .33f) , userPtRad);
		test21 = pa.V(bs01,pt01);
		pt02 = new myClrPoint(pa, "kn1",LABrampPt(p1, p2, .33f) , userPtRad);
		test22 = pa.V(bs01,pt02);
		pt03 = new myClrPoint(pa, "kn1",LCHrampPt(p1, p2, .33f) , userPtRad);
		test23 = pa.V(bs01,pt03);
		
		pt41 = new myClrPoint(pa, "kn2",XYZrampPt(p1, p2, .66f) , userPtRad);
		test31 = pa.V(bs02,pt41);		
		pt42 = new myClrPoint(pa, "kn2",LABrampPt(p1, p2, .66f) , userPtRad);
		test32 = pa.V(bs02,pt42);	
		pt43 = new myClrPoint(pa, "kn2",LCHrampPt(p1, p2, .66f) , userPtRad);
		test33 = pa.V(bs02,pt43);
		
		pt01._add(myVector._mult(test21, .3f));
		pt02._add(myVector._mult(test22, 1.5f));
		pt03._add(myVector._mult(test23, .5f));
		pt41._add(myVector._mult(test31, .3f));
		pt42._add(myVector._mult(test32, 1.5f));
		pt43._add(myVector._mult(test33, .5f));
		
		myPoint	ptu1, ptu2, ptu3;
			
		for(float s = 0; s <= 1.0f; s+=sIncr){		
			float t = PApplet.pow(PApplet.sin(s*.5f*PConstants.PI), 2);
			ptu1 = pa.bezierPoint(new myPoint[] {pa.P(p1),pt01,pt41,pa.P(p2)}, t);
			ptu2 = pa.bezierPoint(new myPoint[] {pa.P(p1),pt02,pt42,pa.P(p2)}, t);
			ptu3 = pa.bezierPoint(new myPoint[] {pa.P(p1),pt03,pt43,pa.P(p2)}, t);
			
			clrPtsArrays[USR1].add(new myClrPoint(pa, "USR1",ptu1, clrPtRad));	
			clrPtsArrays[USR2].add(new myClrPoint(pa, "USR2",ptu2, clrPtRad));
			clrPtsArrays[USR3].add(new myClrPoint(pa, "USR3",ptu3, clrPtRad));
		}	

		for(int i =0; i<numGrads; ++i){
			grads[i].rebuildClrs((myClrPoint[])(clrPtsArrays[i]).toArray(new myClrPoint[0]));
		}		
	}//
...
//	}//drawGradientRectangle
	...
	public double[] LABtoXYZ(double L, double a, double b) {
	      ...
	      double y3 = Math.pow(y, 3.0f);
	      double x = (a / 500.0f) + y;
	      double x3 = Math.pow(x, 3.0f);
	      double z = y - (b / 200.0f);
	      double z3 = Math.pow(z, 3.0f);
	...
	public int[] XYZtoRGB(double X, double Y, double Z) {
	      ...
	      // assume sRGB
	      if (r > 0.0031308f) r = ((1.055f * Math.pow(r, 1.0f / 2.4f)) - 0.055f); else r = (r * 12.92f);
	      if (g > 0.0031308f) g = ((1.055f * Math.pow(g, 1.0f / 2.4f)) - 0.055f); else g = (g * 12.92f);
	      if (b > 0.0031308f) b = ((1.055f * Math.pow(b, 1.0f / 2.4f)) - 0.055f); else b = (b * 12.92f);
	      r = (r < 0) ? 0 : r;
	      r = (r > 1) ? 1 : r;
	      g = (g < 0) ? 0 : g;
	      g = (g > 1) ? 1 : g;
	...
	public void cylinder(int c1, int c2) {
	  myPoint C1 = PofColor(c1);
	  myPoint C2 = PofColor(c2);
	  myPoint P = C1;
	  myVector V = pa.V(C1,C2);
	  myVector I = pa.U(Normal(V));
	  myVector J = pa.U(pa.N(I,V));
	  float da = PConstants.TWO_PI/12;
	  pa.beginShape(PConstants.QUAD_STRIP);
	    for(float a=0; a<=PConstants.TWO_PI+da; a+=da) {
	      pa.fill(c1); pa.gl_vertex(pa.P(P,barThick*Math.cos(a),I,barThick*Math.sin(a),J,0,V)); 
	      pa.fill(c2); pa.gl_vertex(pa.P(P,barThick*Math.cos(a),I,barThick*Math.sin(a),J,1,V));
	      }
	    pa.endShape();
	  }
